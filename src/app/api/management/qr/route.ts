...
import { qrCreateSchema } from "@/server/qr/schemas";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const input = parseInput(qrCreateSchema, await readJson(request));
    return successResponse(await createQrCode(actor, input, context.requestId), { status: 201 });
  } catch (error: unknown) {
